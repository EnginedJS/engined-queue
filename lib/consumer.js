...
module.exports = (opts) => class extends Service {
	...
	constructor(context) {
		...
		this.exchangeOptions = opts.exchangeOptions || {};
	...
	async init() {
		...
		let q = await channel.assertQueue(inputQueue, Object.assign({}, this.queueOptions));

		if (exName !== null) {
			// Initializing exchange
			await channel.assertExchange(exName, this.exchangeType, Object.assign({ durable: false }, this.exchangeOptions));
